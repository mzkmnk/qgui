  ...
  private applyStyles(html: string): string {
    ...
    return (
      html
        ...
        .replace(
          /<pre>/g,
          '<pre class="bg-zinc-900 border border-zinc-700 rounded-lg p-4 overflow-x-auto my-2 language-">'
        )
        ...
        .replace(/<code([^>]*)>/g, (match, attrs) => {
          ...
          if (!match.includes('class="language-')) {
            return (
              '<code class="bg-zinc-800 text-orange-400 px-1 py-0.5 rounded text-sm" ' +
              attrs +
              '>'
            );
          }
        ...
        .replace(
          /<h1>/g,
          '<h1 class="text-xl font-bold text-gray-100 mt-4 mb-2">'
        )
        .replace(
          /<h2>/g,
          '<h2 class="text-lg font-semibold text-gray-100 mt-3 mb-2">'
        )
        .replace(
          /<h3>/g,
          '<h3 class="text-base font-semibold text-gray-200 mt-2 mb-1">'
        )
        // リスト
        .replace(
          /<ul>/g,
          '<ul class="list-disc list-inside space-y-1 my-2 text-gray-200">'
        )
        .replace(
          /<ol>/g,
          '<ol class="list-decimal list-inside space-y-1 my-2 text-gray-200">'
        )
        ...
        .replace(
          /<a /g,
          '<a class="text-orange-400 hover:text-orange-300 underline transition-colors" '
        )
        // 引用
        .replace(
          /<blockquote>/g,
          '<blockquote class="border-l-4 border-orange-500 pl-4 my-2 text-gray-300 italic">'
        )
        // テーブル
        .replace(
          /<table>/g,
          '<table class="min-w-full divide-y divide-zinc-700 my-2">'
        )
